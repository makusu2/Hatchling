...
namespace UnityStandardAssets.Water
{
    ...
    public class Water : MonoBehaviour
    {
        ...
        void OnTriggerEnter(Collider col) {
            ...
            //print("Trigger entered: "+colGO.name);
            WaterEnterer waterEntererGO = colGO.GetComponent<WaterEnterer>(); //GetComponentInChildren, or is this good?
            //print("Detected entrance: "+colGO.name+", specifically "+((MonoBehaviour)waterEntererGO).gameObject.name);
            if(waterEntererGO != null) {
                if(!waterEnterersCurrentlyContained.ContainsKey(waterEntererGO)) {
                    waterEnterersCurrentlyContained[waterEntererGO] = 0;
                }
                waterEnterersCurrentlyContained[waterEntererGO] += 1;
            }
        }
        
        void OnTriggerExit(Collider col) {
            if(IsBottomWater) { return; }
            //print("Trigger exited: "+col.gameObject.name);
            //Maybe after OnTriggerExit, we test the position of the col and if it's lower it's onwaterexit else onwaterenter
            GameObject colGO = col.gameObject;
            //print("Trigger entered: "+colGO.name);
            WaterEnterer waterEntererGO = colGO.GetComponent<WaterEnterer>(); //GetComponentInChildren, or is this good?
            //print("Detected entrance: "+colGO.name+", specifically "+((MonoBehaviour)waterEntererGO).gameObject.name);
            if(waterEntererGO != null) {
                //print("Water enterer GO wasn't null for: "+colGO.name);
                waterEnterersCurrentlyContained[waterEntererGO] -= 1;
                bool allContactPointsOut = waterEnterersCurrentlyContained[waterEntererGO] == 0;
                if(allContactPointsOut) {
                    bool colBelow = col.gameObject.transform.position.y < transform.position.y;
                    if(colBelow) {
                        waterEntererGO.OnWaterEnter();
                    }
                    else {
                        waterEntererGO.OnWaterExit();
                    }
                }
            }
        }
